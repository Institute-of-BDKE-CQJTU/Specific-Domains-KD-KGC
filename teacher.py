...
def train(model, triples, ent_num):
    ...
    train_triples, valid_triples, test_triples, rule_triples = triples
    all_true_triples = train_triples + valid_triples + test_triples + rule_triples
    r_tp = rel_type(train_triples+rule_triples)
    ...
    test_data_list = test_data_sets(valid_triples+rule_triples, true_all_heads, true_all_tails, ent_num, r_tp)
...
def run():
    ...
    symmetry_rule = read_triples(os.path.join(config.data_path, "symmetry_rule.txt"), ent2id, rel2id)
    reflexive_rule = read_triples(os.path.join(config.data_path, "reflexive_rule.txt"), ent2id, rel2id)
    transitive_rule = read_triples(os.path.join(config.data_path, "transitive_rule.txt"), ent2id, rel2id)
    equivalent_rule = read_triples(os.path.join(config.data_path, "equivalent_rule.txt"), ent2id, rel2id)
    sub_rule = read_triples(os.path.join(config.data_path, " sub_rule.txt"), ent2id, rel2id)
    two_hop_rule = read_triples(os.path.join(config.data_path, "two_hop_rule.txt"), ent2id, rel2id)
    three_hop_rule = read_triples(os.path.join(config.data_path, "three_hop_rule.txt"), ent2id, rel2id)
    inverse_rule = read_triples(os.path.join(config.data_path, "inverse_rule.txt"), ent2id, rel2id)
    rule_set = symmetry_rule + reflexive_rule + transitive_rule+equivalent_rule+sub_rule+two_hop_rule+three_hop_rule+inverse_rule
    ...
    logging.info("#rule triple num: %d" % len(rule_set))
