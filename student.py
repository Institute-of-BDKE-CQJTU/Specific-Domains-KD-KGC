...
from model import TransE, TransH, TransD, STransE, LineaRE, DistMult, ComplEx, RotatE, SimpleTransR, TransIJ, \
    LineaRE_rule_matched
from utils import set_logger, read_elements, read_triples, log_metrics, save_model, train_data_iterator, \
    get_optim, train_step, test_step, rel_type, test_data_sets, get_true_ents, train_step_with_teacher_supervise
...
def train(model, triples, ent_num):
    ...
    max_mrr = 0.0
    training_logs = []
    student_modes = ["Prediction Head", "Prediction Tail"]
    teacher_modes = ["Prediction Head", "Prediction Tail"]
    checkpoint = torch.load(os.path.join(config.save_path, "checkpoint"))
    teacher_modes.load_state_dict(checkpoint["model_state_dict"])
    rtps = ["1-1"]
    # Training Loop
    for step in range(init_step, config.max_step + 1):
        log = train_step_with_teacher_supervise(student_modes, teacher_modes, optimizer, next(train_iterator))
...
def run():
    ...
    if config.model == "TransH":
        ...
        kge_model_teacher = LineaRE_rule_matched(ent_num, rel_num)
    ...
    train(
        model=kge_model,
        teacher_model=kge_model_teacher,
        triples=(
            train_triples,
            valid_triples,
            test_triples,
