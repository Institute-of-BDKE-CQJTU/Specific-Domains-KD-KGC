...
def train_step_with_teacher_supervise(model_teacher, model_student, optimizer, data):
    model_student.train()
    model_teacher.eval()
    optimizer.zero_grad()
    ...
    pos_score = model_student(pos_sample)
    neg_score = model_student(pos_sample, neg_sample, mode)
    ...
    loss_1 = (pos_sample_loss + neg_sample_loss) / 2
    loss_2 = 0
    my_lambda = 0.5
    regularization_log = {}
    if config.model == "TransE":
        stu_ent_reg = torch.abs(model_student.ent_embd.weight.norm(dim=-1) - 1).mean()
        tea_ent_reg = torch.abs(model_teacher.ent_embd.weight.norm(dim=-1) - 1).mean()
        loss_2 += scipy.stat.entropy(stu_ent_reg, tea_ent_reg)
        loss_1 += stu_ent_reg * config.regularization
        loss = my_lambda * loss_1 + (1 - my_lambda) * loss_2
        regularization_log["ent_reg"] = stu_ent_reg.item()
    elif config.model == "TransH":
        stu_ent_reg = torch.relu(model_student.ent_embd.weight.norm(dim=-1) - 1).mean()
        wr_norm_1 = torch.abs(model_student.wr.weight.norm(dim=-1) ** 2 - 1.0).mean()
        wrr1 = func.relu(
            (
                    (model_student.wr.weight * model_student.rel_embd.weight).sum(dim=-1)
                    / model_student.rel_embd.weight.norm(dim=-1)
            ) ** 2
            - 0.0001
        ).mean()
        loss_1 += wr_norm_1 * config.regularization
        loss_1 += wrr1 * config.regularization
        wr_norm_2 = torch.abs(model_teacher.wr.weight.norm(dim=-1) ** 2 - 1.0).mean()
        wrr2 = func.relu(
            (
                    (model_teacher.wr.weight * model_teacher.rel_embd.weight).sum(dim=-1)
                    / model_teacher.rel_embd.weight.norm(dim=-1)
            ) ** 2
            - 0.0001
        ).mean()
        loss_2 += wr_norm_2 * config.regularization
        loss_2 += wrr2 * config.regularization
        loss = my_lambda * loss_1 + (1 - my_lambda) * loss_2
        regularization_log["ent_reg"] = stu_ent_reg.item()
        regularization_log["wr_norm_1"] = wr_norm_1.item()
        regularization_log["wrr_reg"] = wrr1.item()
    elif config.model == "DistMult":
        stu_ent_reg = torch.sum(model_student.ent_embd.weight ** 2, dim=-1).mean()
        stu_rel_reg = torch.sum(model_student.rel_embd.weight ** 2, dim=-1).mean()
        loss_1 += stu_ent_reg * config.regularization
        loss_1 += stu_rel_reg * config.regularization

        tea_ent_reg = torch.sum(model_teacher.ent_embd.weight ** 2, dim=-1).mean()
        tea_rel_reg = torch.sum(model_teacher.rel_embd.weight ** 2, dim=-1).mean()
        loss_2 += tea_ent_reg * config.regularization
        loss_2 += tea_rel_reg * config.regularization
        regularization_log["ent_reg"] = stu_ent_reg.item()
        regularization_log["rel_reg"] = stu_rel_reg.item()

        loss = my_lambda * loss_1 + (1 - my_lambda) * loss_2
    elif config.model == "ComplEx":
        stu_ent_reg = torch.sum(model_student.ent_embd.weight ** 2, dim=-1).mean()
        rel_reg = torch.sum(model_student.rel_embd.weight ** 2, dim=-1).mean()
        ent_im_reg = torch.sum(model_student.ent_embd_im.weight ** 2, dim=-1).mean()
        rel_im_reg = torch.sum(model_student.rel_embd_im.weight ** 2, dim=-1).mean()
        loss_1 += stu_ent_reg * config.regularization
        loss_1 += rel_reg * config.regularization
        loss_1 += ent_im_reg * config.regularization
        loss_1 += rel_im_reg * config.regularization
        regularization_log["ent_reg"] = stu_ent_reg.item()
        ...
        stu_ent_reg = torch.sum(model_student.ent_embd.weight ** 2, dim=-1)
        rel_reg = torch.sum(model_student.rel_embd.weight ** 2, dim=-1)
        reg = torch.cat([stu_ent_reg, rel_reg]).mean()
        loss_1 += reg * config.regularization
        ent_p_reg = torch.sum(model_student.ent_p.weight ** 2, dim=-1)
        rel_p_reg = torch.sum(model_student.rel_p.weight ** 2, dim=-1)
        reg = torch.cat([ent_p_reg, rel_p_reg]).mean()
        loss_1 += reg * config.regularization
        regularization_log["ent_reg"] = stu_ent_reg.mean().item()
        ...
        stu_ent_reg = torch.sum(model_student.ent_embd.weight ** 2, dim=-1)
        stu_rel_reg = torch.sum(model_student.rel_embd.weight ** 2, dim=-1)
        stu_reg = torch.cat([stu_ent_reg, stu_rel_reg]).mean()
        loss_1 += stu_reg * config.regularization
        regularization_log["ent_reg"] = stu_ent_reg.mean().item()
        regularization_log["rel_reg"] = stu_reg.mean().item()

        tea_ent_reg = torch.sum(model_teacher.ent_embd.weight ** 2, dim=-1)
        tea_rel_reg = torch.sum(model_teacher.rel_embd.weight ** 2, dim=-1)
        tea_reg = torch.cat([tea_ent_reg, tea_rel_reg]).mean()
        loss_2 += tea_reg * config.regularization
...
def rule_match(heads, relations, tails, rule):
    symmetric_rule, reflexive_rule, inverse_rule, sub_rule, \
    transitive_rule, equivalent_rule, two_hop_rule, three_hop_rule = rule
    teacher_triplets = []
    relation_weights = []
    for head_id, relation_id, tail_id in zip(heads.tolist(), relations.tolist(), tails.tolist()):
        if relation_id in reflexive_rule:
            teacher_triplets.append([head_id, relation_id, head_id])
            relation_weights.append(1.0)
        elif relation_id in symmetric_rule:
            teacher_triplets.append([tail_id, relation_id, head_id])
            relation_weights.append(1.0)
        elif relation_id in inverse_rule.keys():
            teacher_triplets.append([tail_id, choice(inverse_rule[relation_id]), head_id])
            relation_weights.append(1.0)
        elif relation_id in sub_rule.keys():
            teacher_triplets.append([head_id, choice(sub_rule[relation_id]), tail_id])
            relation_weights.append(1.0)
        elif relation_id in equivalent_rule.keys():
            teacher_triplets.append([head_id, choice(equivalent_rule[relation_id]), tail_id])
            relation_weights.append(1.0)
        elif relation_id in transitive_rule.keys():
            teacher_triplets.append([head_id, choice(transitive_rule[relation_id]), tail_id])
            relation_weights.append(1.0)
        elif relation_id in two_hop_rule.keys():
            teacher_triplets.append([head_id, choice(two_hop_rule[relation_id][0]), tail_id])
            teacher_triplets.append([head_id, choice(two_hop_rule[relation_id][1]), tail_id])
            relation_weights.append(1.0)
        elif relation_id in three_hop_rule.keys():
            teacher_triplets.append([head_id, choice(three_hop_rule[relation_id][0]), tail_id])
            teacher_triplets.append([head_id, choice(three_hop_rule[relation_id][1]), tail_id])
            teacher_triplets.append([head_id, choice(three_hop_rule[relation_id][2]), tail_id])
            relation_weights.append(1.0)
        else:  # 无规则匹配情形
            teacher_triplets.append([1, 1, 1])
            relation_weights.append(0.0)
    return teacher_triplets, relation_weights
